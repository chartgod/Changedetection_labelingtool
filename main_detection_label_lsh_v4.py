# author: LSH <Change detection tool 폴리곤 작업>
import pandas as pd
import numpy as np
import os
import sys
...
from PyQt5.QtGui import (
    QPainter, QPen, QColor, QBrush, QPolygon, QPixmap, QIntValidator, QCursor, QMouseEvent, QFont
)
    ...
    def paintEvent(self, e):
        ...
        try:
            ...
            if self.img:
                ...
                # 미완성된 선 그리기
                if self.is_drawing and self.pos:
                    # 현재 클래스에 따른 펜 색상 설정
    ...
    def mouseReleaseEvent(self, e):
        try:
            ...
            if e.button() == Qt.RightButton and not self.is_moving and self.is_drawing:
                ...
                undo_act = QAction(u"Undo Ctrl+Z", self,
...
class change_detection(QMainWindow):
    def __init__(self, parent=None):
        ...
        edit.addAction("Question : dlgkstn68@naver.com, LEE-SEUNG-HEON")
        edit.addAction("URL : https://github.com/chartgod/Changedetection_labelingtool")
    ...
    def lv_loadimg(self, qModelIndex):
        try:
            ...
            # 새로운 이미지에 대한 라벨 로드
            self.box.poly_list = self.image_labels.get(self.box.path, []).copy()
    ...
    def load_labels_from_file(self):
        ...
        if os.path.exists(csv_filepath):
            ...
            # Read the file and calculate the maximum number of coordinates
            with open(csv_filepath, 'r', encoding='utf_8_sig') as f:
                content = f.readlines()
                for i in range(1, len(content), 2):
                    points_str = content[i].strip().split(',')
                    max_points = max(max_points, len(points_str))

            # Load the polygons and pad with NaN where necessary
            with open(csv_filepath, 'r', encoding='utf_8_sig') as f:
                content = f.readlines()
                for i in range(0, len(content), 2):
                    ...
                    # Pad the points with NaN if they have fewer than max_points coordinates
    ...
    def lv_loadimgB(self, qModelIndex):
        try:
            # Temporary B 이미지 선택 시 경로 저장
            self.selected_b_image_path = self.temp_listB[qModelIndex.row()]
            if not self.temp_listA:
                QMessageBox.information(self, "경고", "Temporary A를 먼저 불러오세요")
            else:
            ...
            print(f"lv_loadimgB 오류: {e}")
            QMessageBox.warning(self, "오류", f"이미지 B 로드 실패: {e}")
    ...
    def openimage(self, flag):
        try:
            ...
            self.undo_stack.clear()
            self.redo_stack.clear()
            imgName, _ = QFileDialog.getOpenFileNames(self, "파일 선택", "",
            ...
            if flag == "A":
                ...
                self.set_list()
                if self.temp_listB:
                    self.box.imgB = QPixmap(self.temp_listB[0])
        except Exception as e:
    ...
    def savepoint(self):
        try:
            if self.box.path is None:
                QMessageBox.information(self, "경고", "라벨을 저장할 이미지가 없습니다.")
                return

            (filepath, filename) = os.path.split(self.box.path)
            csv_filename = os.path.splitext(filename)[0] + "_GT.csv"  # Save as .csv file
            ...
            with open(os.path.join(label_dir, csv_filename), 'w', encoding='utf_8_sig') as f:
                for poly_dict in self.box.poly_list:
                    ...
                    point_str = ','.join(map(str, points))  # Write each coordinate separated by a comma
                    f.write(f"{point_str}\n")
